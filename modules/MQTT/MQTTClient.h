...
int MQTT::Client<Network, Timer, a, b>::sendPacket(int length, Timer& timer)
{
    ...
    /*
    if(rc == FAILURE) {
        ERR("Rc %d from sending packet \n", rc );
    } else {
        DBG("Rc %d from sending packet \n", rc );
    }
    */
    return rc;
}
...
{
    int rc = FAILURE;
    //DBG("keepalive (interval %d)", keepAliveInterval);
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, MAX_MESSAGE_HANDLERS>::subscribe(const char* topicFilter, enum QoS qos, messageHandler messageHandler)
{
    ...
    len = MQTTSerialize_subscribe(sendbuf, MAX_MQTT_PACKET_SIZE, 0, packetid.getNext(), 1, &topic, &tmp_qos);
    if (len <= 0)
        goto exit;
