...
#if 1
#define DBG(...) do{printf("%p %d %s ", this,__LINE__,__PRETTY_FUNCTION__); debug(__VA_ARGS__); } while(0);
//#define DBG(x, ...) debug("[W5500:DBG]"x"\r\n", ##__VA_ARGS__);
#define WARN(x, ...) printf("[W5500:WARN]"x"\r\n", ##__VA_ARGS__);
#define ERR(x, ...) printf("[W5500:ERR]"x"\r\n", ##__VA_ARGS__);
#else
#define DBG(x, ...)
#define WARN(x, ...)
#define ERR(x, ...)
...
#if 1
#define INFO(x, ...) printf("[W5500:INFO]"x"\r\n", ##__VA_ARGS__);
#else
#define INFO(x, ...)
#endif
...
namespace W5500
{
    ...
    {
        printf("Resetting wizet interface...\n\r");
        reset_pin = 1;
        reset_pin = 0;
    ...
    #if DBG_SPI
        printf("[SPI]W %04x(%02x %d)", addr, cb, len);
        for(int i = 0; i < len; i++) {
            printf(" %02x", buf[i]);
            if (i > 16) {
                printf(" ...");
    ...
    #if DBG_SPI
        printf("[SPI]R %04x(%02x %d)", addr, cb, len);
        for(int i = 0; i < len; i++) {
            printf(" %02x", buf[i]);
            if (i > 16) {
                printf(" ...");
